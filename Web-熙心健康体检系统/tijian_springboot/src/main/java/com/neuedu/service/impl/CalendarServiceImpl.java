...
public class CalendarServiceImpl implements CalendarService {
    ...
    public List<CalendarResponseDto> list(CalendarRequestDto calendarRequestDto) {
        ...
        int year=calendarRequestDto.getYear(), month= calendarRequestDto.getMonth();
        boolean isRun = false;
        if((year%4==0&&year%100!=0) || (year%400==0)) {
            isRun = true;
        }
        ...
        Date now = new Date();
        Date firstDateOfMonth = new Date(year-1900,month-1,1);
        int day = firstDateOfMonth.getDay();
        for(;day>0;day--)
        {
        ...
        for(int i=1;i<=totalDate;i++) {
            CalendarResponseDto dto= new CalendarResponseDto(year+"-"+month+"-"+i);
